...
export default class DWTOutPut extends React.Component {
  ...
  componentDidUpdate(prevProps) {
    if (this.props.bNoScroll) this.DWTOutPut_message.current.scrollTop = 0;
    else
      this.DWTOutPut_message.current.scrollTop =
        this.DWTOutPut_message.current.scrollHeight;
  }
  handleKeyUp(e) {
  ...
  render() {
    return (
      <div className="DWTOutPut">
        Message: {this.props.note}
        <br />
        <div
          ref={this.DWTOutPut_message}
          tabIndex="8"
          className="message"
          onKeyUp={(e) => this.handleKeyUp(e)}
          onDoubleClick={() => this.props.handleEvent("doubleClick")}
        >
          <ul>
            {this.props.messages.map((oneMsg) => (
              <li
                key={oneMsg.time + "_" + Math.floor(Math.random(1) * 10000000)}
                className={oneMsg.type}
              >
                {oneMsg.text}
              </li>
            ))}
