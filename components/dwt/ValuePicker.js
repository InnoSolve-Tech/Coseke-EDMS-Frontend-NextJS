...
export default class ValuePicker extends React.Component {
  ...
  componentDidUpdate(prevProps) {
    ...
    if (
      ...
      this.props.targetObject !== prevProps.targetObject
    ) {
      for (let i = 0; i < _valuePacks.length; i++) {
        if (_valuePacks[i].name === this.props.current) {
          this.setState({ currentValues: _valuePacks[i].items }, () =>
  ...
  changeScrollTop() {
    let _valueLIs = this.valuesList.current.children,
      _scrollTop = 0,
      _height = 0;
    ...
    if (this.valuesList.current.scrollTop - _scrollTop > 0)
      ...
      this.valuesList.current.scrollTop + this.valuesList.current.offsetHeight <
      _scrollTop + _height
    ) {
      this.valuesList.current.scrollTop =
        _scrollTop + _height - this.valuesList.current.offsetHeight;
    }
  }
  handlePackChange(event) {
    ...
    for (let i = 0; i < this.props.valuePacks.length; i++) {
      if (this.props.valuePacks[i].name === packName) {
        this.setState({
          currentValues: this.props.valuePacks[i].items,
          current: packName,
        });
        break;
      }
  ...
  handleValuePicked(event) {
    ...
    for (let i = 0; i < old_Values.length; i++) {
      if (old_Values[i].value === value) old_Values[i].checked = true;
      else old_Values[i].checked = false;
      this.setState({ currentValues: old_Values }, () => {
        this.props.handleValuePicking({
          prop: this.state.current,
          value: value,
        });
      });
    }
