...
export default class DWTController extends React.Component {
  constructor(props) {
    ...
    if (this.props.features & (7 === 0)) {
      ...
      this.initialShownTabs =
        this.props.features & 1 ||
        this.props.features & 2 ||
        this.props.features & 4;
  ...
  handleTabs(event) {
    ...
    if (this.state.shownTabs & nControlIndex) {
      //close a Tab
  ...
  onCameraChange(value) {
    ...
    if (this.DWTObject.Addon.Webcam.SelectSource(value)) {
      ...
      _advancedSettings = Object.keys(
        this.Dynamsoft.DWT.EnumDWT_VideoProperty,
      ).map((_value) => {
        return { value: _value.substr(3) };
      });
      _advancedCameraSettings = Object.keys(
        this.Dynamsoft.DWT.EnumDWT_CameraControlProperty,
      ).map((_value) => {
        return { value: _value.substr(4) };
      });
      this.setState({
        cameraSettings: [
          {
      ...
      this.props.handleException({
        ...
        message:
          ...
          value +
          ", please make sure it's not in use!",
      });
      if (!this.cameraReady) {
        this.cameraReady = true;
        this.props.handleStatusChange(2);
    ...
    if (config) {
      if (config.prop === "Video Setup" || config.prop === "Camera Setup") {
        ...
        if (config.prop === "Video Setup") {
          bCamera = false;
          basicSetting = this.DWTObject.Addon.Webcam.GetVideoPropertySetting(
            this.Dynamsoft.DWT.EnumDWT_VideoProperty["VP_" + config.value],
          );
          moreSetting = this.DWTObject.Addon.Webcam.GetVideoPropertyMoreSetting(
            this.Dynamsoft.DWT.EnumDWT_VideoProperty["VP_" + config.value],
          );
        } else {
          basicSetting =
            this.DWTObject.Addon.Webcam.GetCameraControlPropertySetting(
              this.Dynamsoft.DWT.EnumDWT_CameraControlProperty[
                "CCP_" + config.value
              ],
            );
          moreSetting =
            this.DWTObject.Addon.Webcam.GetCameraControlPropertyMoreSetting(
              this.Dynamsoft.DWT.EnumDWT_CameraControlProperty[
                "CCP_" + config.value
              ],
            );
        ...
        switch (config.prop) {
          case "Frame Rate":
            this.DWTObject.Addon.Webcam.SetFrameRate(config.value);
            break;
          case "Media Type":
            this.DWTObject.Addon.Webcam.SetMediaType(config.value);
            break;
          case "Resolution":
            this.DWTObject.Addon.Webcam.SetResolution(config.value);
            break;
          default:
            break;
        }
  ...
  captureImage() {
    if (this.DWTObject) {
      ...
      this.DWTObject.Addon.Webcam.CaptureImage(
        funCaptureImage,
        funCaptureImage,
      );
    }
  ...
  handleSaveConfigChange(event) {
    ...
    switch (format) {
      default:
        break;
      case "multiPage":
        this.setState({ bMulti: event.target.checked });
        break;
      case "tif":
      case "pdf":
        this.setState({ saveFileFormat: event.target.value, bMulti: true });
        break;
      case "bmp":
      case "jpg":
      case "png":
        this.setState({ saveFileFormat: event.target.value, bMulti: false });
        break;
    }
  }
  ...
  saveOrUploadImage(_type) {
    ...
    let onFailure = (errorCode, errorString, httpResponse) => {
      httpResponse && httpResponse !== ""
        ? this.props.handleOutPutMessage(httpResponse, "httpResponse")
        : this.props.handleException({ code: errorCode, message: errorString });
    };
    if (this.state.bMulti) {
      if (
        this.props.selected.length === 1 ||
        this.props.selected.length === this.props.buffer.count
      ) {
        if (_type === "local") {
          switch (this.state.saveFileFormat) {
            default:
              break;
            case "tif":
              this.DWTObject.SaveAllAsMultiPageTIFF(
                fileName,
                onSuccess,
                onFailure,
              );
              break;
            case "pdf":
              this.DWTObject.SaveAllAsPDF(fileName, onSuccess, onFailure);
              break;
          }
        } else {
        ...
        if (_type === "local") {
          switch (this.state.saveFileFormat) {
            default:
              break;
            case "tif":
              this.DWTObject.SaveSelectedImagesAsMultiPageTIFF(
                fileName,
                onSuccess,
                onFailure,
              );
              break;
            case "pdf":
              this.DWTObject.SaveSelectedImagesAsMultiPagePDF(
                fileName,
                onSuccess,
                onFailure,
              );
    ...
    for (let o in this.Dynamsoft.DWT.EnumDWT_ImageType) {
      if (
        o.toLowerCase().indexOf(this.state.saveFileFormat) !== -1 &&
        this.Dynamsoft.DWT.EnumDWT_ImageType[o] < 7
      ) {
    ...
    if (_type === "server") {
      ...
      let serverUrl =
        protocol + window.location.hostname + ":" + _strPort + strActionPage;
      if (this.state.bUseFileUploader) {
        ...
        this.DWTObject.GenerateURLForUploadData(
          imagesToUpload,
          fileType,
  ...
  initBarcodeReader(_features) {
    this.DWTObject.Addon.BarcodeReader.getRuntimeSettings().then(
      (settings) => {
  ...
  readBarcode() {
    ...
    this.DWTObject.Addon.BarcodeReader.getRuntimeSettings().then((settings) => {
      if (this.DWTObject.GetImageBitDepth(this.props.buffer.current) === 1)
        settings.scaleDownThreshold = 214748347;
      else settings.scaleDownThreshold = 2300;
      settings.barcodeFormatIds = this.Dynamsoft.DBR.EnumBarcodeFormat.BF_ALL;
      settings.barcodeFormatIds_2 =
        this.Dynamsoft.DBR.EnumBarcodeFormat_2.BF2_DOTCODE +
        this.Dynamsoft.DBR.EnumBarcodeFormat_2.BF2_POSTALCODE;
      ...
      if (this.props.zones.length > 0) {
        ...
        let readBarcodeFromRect = () => {
          ...
          settings.region.right =
            this.props.zones[i].x + this.props.zones[i].width;
          settings.region.bottom =
            this.props.zones[i].y + this.props.zones[i].height;
          if (i === this.props.zones.length - 1) this.doReadBarode(settings);
          else this.doReadBarode(settings, readBarcodeFromRect);
        };
        settings.region.left = this.props.zones[0].x;
        settings.region.top = this.props.zones[0].y;
        settings.region.right =
          this.props.zones[0].x + this.props.zones[0].width;
        settings.region.bottom =
          this.props.zones[0].y + this.props.zones[0].height;
        if (this.props.zones.length === 1) this.doReadBarode(settings);
        else this.doReadBarode(settings, readBarcodeFromRect);
      } else {
  ...
  doReadBarode(settings, callback) {
    ...
    this.DWTObject.Addon.BarcodeReader.updateRuntimeSettings(settings).then(
      (settings) => {
        ...
        let outputResults = () => {
          if (this.dbrResults.length === 0) {
            ...
            this.props.handleOutPutMessage(
              "Total barcode(s) found: " + this.dbrResults.length,
              "important",
            );
            for (let i = 0; i < this.dbrResults.length; ++i) {
              ...
              this.props.handleOutPutMessage(
                "Type: " + result.BarcodeFormatString,
              );
              this.props.handleOutPutMessage(
        ...
        this.DWTObject.Addon.BarcodeReader.decode(
          this.props.buffer.current,
        ).then(onDbrReadSuccess, onDbrReadFail);
      },
    );
  }
  doneReadingBarcode() {
  ...
  handleRangeChange(event) {
    ...
    if (value === "reset-range") {
      ...
      _type === "camera"
        ? this.DWTObject.Addon.Webcam.SetCameraControlPropertySetting(
            this.Dynamsoft.DWT.EnumDWT_CameraControlProperty["CCP_" + prop],
            _default,
            true,
          )
        : this.DWTObject.Addon.Webcam.SetVideoPropertySetting(
            this.Dynamsoft.DWT.EnumDWT_VideoProperty["VP_" + prop],
            _default,
            true,
          );
          ...
          <ul className="PCollapse">
            {this.props.features & 0b1 ? (
              <li>
                <div
                  ...
                  tabIndex="1"
                  controlindex="1"
                  onKeyUp={(event) => this.handleTabs(event)}
                  onClick={(event) => this.handleTabs(event)}
                >
                  <div
                    className={
                      this.state.shownTabs & 1
                        ? "mark_arrow expanded"
                        : "mark_arrow collapsed"
                    }
                  ></div>
                  Custom Scan
                </div>
                <div
                  className="divTableStyle"
                  style={
                    this.state.shownTabs & 1
                      ? { display: "block" }
                      : { display: "none" }
                  ...
                  <ul>
                      ...
                      <ul>
                        <li>
                          {this.state.deviceSetup.noUI ? (
                            ...
                            <label style={{ width: "32%", marginRight: "2%" }}>
                              <input
                                ...
                                onChange={(e) =>
                                  this.handleScannerSetupChange(e, "bShowUI")
                                }
                              />
                              Show UI&nbsp;
                            </label>
                          ...
                          <label style={{ width: "32%", marginRight: "2%" }}>
                            <input
                              ...
                              onChange={(e) =>
                                this.handleScannerSetupChange(e, "bADF")
                              }
                            />
                            Page Feeder&nbsp;
                          </label>
                          <label style={{ width: "32%" }}>
                            <input
                              ...
                              onChange={(e) =>
                                this.handleScannerSetupChange(e, "bDuplex")
                              }
                            />
                            Duplex
                          </label>
                        ...
                        <li>
                          <select
                            ...
                            onChange={(e) =>
                              this.handleScannerSetupChange(e, "nPixelType")
                            }
                          >
                          ...
                          <select
                            ...
                            onChange={(e) =>
                              this.handleScannerSetupChange(e, "nResolution")
                            }
                          >
                    ...
                    <li className="tc">
                      <button
                        ...
                        className={
                          this.state.scanners.length > 0
                            ? "majorButton enabled fullWidth"
                            : "majorButton disabled fullWidth"
                        }
                        onClick={() => this.acquireImage()}
                        disabled={
                          this.state.scanners.length > 0 ? "" : "disabled"
                        }
                      >
                        Scan
                      </button>
                ...
                >
                  <ul>
                    <li>
                      <select
                        tabIndex="2"
                        value={this.state.deviceSetup.currentCamera}
                        className="fullWidth"
                        onChange={(e) => this.onCameraChange(e.target.value)}
                      >
                        {this.state.cameras.length > 0 ? (
                          this.state.cameras.map((_name, _index) => (
                            <option value={_index} key={_index}>
                              {_name}
                            </option>
                          ))
                        ) : (
                          <option value="nocamera">
                            Looking for devices..
                          </option>
                        )}
                      </select>
                      {this.state.cameraSettings.length > 0 ? (
                        <ValuePicker
                          ...
                          current={"Resolution"}
                          handleValuePicking={(valuePair) =>
                            this.playVideo(valuePair)
                          }
                        />
                      ) : (
                        ""
                    ...
                    <li className="tc">
                      <button
                        tabIndex="2"
                        className="majorButton enabled width_48p"
                        onClick={() => this.toggleShowVideo()}
                      >
                        {this.state.deviceSetup.isVideoOn
                          ? "Hide Video"
                          : "Show Video"}
                      </button>
                      <button
                        ...
                        className={
                          this.state.deviceSetup.isVideoOn
                            ? "majorButton enabled width_48p marginL_2p"
                            : "majorButton disabled width_48p marginL_2p"
                        }
                        onClick={() => this.captureImage()}
                        disabled={
                          this.state.deviceSetup.isVideoOn ? "" : "disabled"
                        }
                      >
                        {" "}
                        Capture
            ...
            {this.props.features & 0b1000 || this.props.features & 0b10000 ? (
              <li>
                ...
                <div
                  className="divTableStyle div_SaveImages"
                  style={
                    this.state.shownTabs & 8
                      ? { display: "block" }
                      : { display: "none" }
                ...
                >
                  <ul>
                    ...
                    <li>
                      <label>
                        <input
                          ...
                          value="bmp"
                          name="ImageType"
                          onClick={(e) => this.handleSaveConfigChange(e)}
                        />
                        BMP
                      </label>
                      <label>
                    ...
                    <li>
                      <label>
                        <input
                          ...
                          type="checkbox"
                          checked={
                            (this.state.saveFileFormat === "pdf" ||
                              this.state.saveFileFormat === "tif") &&
                            (this.state.bMulti ? "checked" : "")
                          }
                          value="multiPage"
                          disabled={
                            this.state.saveFileFormat === "pdf" ||
                          ...
                          onChange={(e) => this.handleSaveConfigChange(e)}
                        />
                        Upload Multiple Pages
                      </label>
                      {this.props.features & 0b10000 &&
                      this.props.features & 0b1000000 ? (
                        <label>
                          <input
                            tabIndex="4"
                            title="Use Uploader"
                            type="checkbox"
                            onChange={(e) => this.toggleUseUploade(e)}
                          />
                          Use File Uploader
                        </label>
                      ) : (
                    ...
                    <li className="tc">
                      {this.state.bWin && this.props.features & 0b1000 ? (
                        <button
                          tabIndex="4"
                          className={
                            this.props.buffer.count === 0
                              ? "majorButton disabled width_48p"
                              : "majorButton enabled width_48p"
                          }
                          disabled={
                            this.props.buffer.count === 0 ? "disabled" : ""
                          }
                          onClick={() => this.saveOrUploadImage("local")}
                        >
                          Save to Local
                        </button>
                      ) : (
                      ...
                      {this.props.features & 0b10000 ? (
                        <button
                          ...
                          className={
                            this.props.buffer.count === 0
                              ? "majorButton disabled width_48p marginL_2p"
                              : "majorButton enabled width_4p marginL_2p"
                          }
                          disabled={
                            this.props.buffer.count === 0 ? "disabled" : ""
                          }
                  ...
                  <ul>
                    <li className="tc">
                      {this.props.features & 0b100000 ? (
                        <button
                          ...
                          disabled={
                            this.props.buffer.count === 0 ||
                            this.state.readingBarcode
                              ? "disabled"
                              : ""
                          }
                          onClick={() => this.readBarcode()}
                        >
                          {this.state.readingBarcode
                            ? "Reading..."
                            : "Read Barcode"}
                    ...
                    {this.props.barcodeRects.length > 0 && (
                      <li>
                        <button
                          tabIndex="5"
                          className="majorButton enabled fullWidth"
                          onClick={() =>
                            this.props.handleBarcodeResults("clear")
                          }
                        >
